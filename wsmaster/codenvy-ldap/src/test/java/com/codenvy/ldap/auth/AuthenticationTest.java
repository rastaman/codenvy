...
public class AuthenticationTest {
  ...
  private static void mustAuthenticate(
      AuthenticationHandler handler, String name, String password) {
    try {
      handler.authenticate(name, password);
    } catch (AuthenticationException x) {
      fail(
          format(
              "Failed to authenticate user '%s' with password '%s' due to an error: %s",
              name, password, x.getMessage()));
    }
  }

  private static void mustNotAuthenticate(
      AuthenticationHandler handler, String name, String password) {
    try {
      handler.authenticate(name, password);
      fail(
          format(
              "Authentication must fail for user '%s' and password '%s', but it was successful",
              name, password));
