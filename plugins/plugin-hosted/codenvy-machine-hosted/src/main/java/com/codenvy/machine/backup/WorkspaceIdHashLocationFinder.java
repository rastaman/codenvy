 ...
 *
 * <p>We can have a lot of workspace and create root folder for all of them at the same level of
 * filesystem may be inefficient. Keep workspace root folder in some hierarchy tree, use hash code
 * for it. <br>
 * Algorithm of evaluation path by workspace ID:
 * <li>Get hashcode of workspace ID
 * <li>For 3 lowest bytes starting from lowest
 * <li>Convert byte to HEX number
 * <li>Use it as parent folder for workspace <br>
 *     Example:
 ...
 * <li>Convert to HEX :94 So 2E, 9A, 94 will be parents of workspace folder. <br>
 *     Path to workspace: <provided root folder>/2e/9a/94/workspaceibi5g3ofwxl52qiq <br>
 *     For workspace with id workspaceq23dfgh6543fh75t path will be <provided root
 *     folder>/76/aa/81/workspaceq23dfgh6543fh75t
 *
 * @author andrew00x
...
public class WorkspaceIdHashLocationFinder {
  ...
  public File calculateDirPath(File parent, String workspaceId) {
    ...
    final String relPath =
        segments[hash & 0xff]
            + File.separatorChar
            + segments[(hash >> 8) & 0xff]
            + File.separatorChar
            + segments[(hash >> 16) & 0xff]
            + File.separatorChar
            + workspaceId;
