...
public class CodenvyDockerServerEvaluationStrategyTest {
  ...
  private CodenvyDockerServerEvaluationStrategy strategy =
      new CodenvyDockerServerEvaluationStrategy();
  ...
  public void shouldUseProvidedInternalHostOnRetrievalOfInternalAddresses() throws Exception {
    ...
    exposedPorts.put(
        "8080/tcp", singletonList(new PortBinding().withHostIp("127.0.0.1").withHostPort("32789")));
    exposedPorts.put(
        "9090/udp",
        singletonList(new PortBinding().withHostIp("192.168.0.1").withHostPort("20000")));
    ...
    for (Map.Entry<String, List<PortBinding>> entry : exposedPorts.entrySet()) {
      assertEquals(
          internalAddressesAndPorts.get(entry.getKey()),
  ...
  public void shouldUseProvidedInternalHostOnRetrievalOfExternalAddresses() throws Exception {
    ...
    for (Map.Entry<String, List<PortBinding>> entry : exposedPorts.entrySet()) {
      assertEquals(
          externalAddressesAndPorts.get(entry.getKey()),
          HOST + ":" + entry.getValue().get(0).getHostPort());
    }
  }
}
