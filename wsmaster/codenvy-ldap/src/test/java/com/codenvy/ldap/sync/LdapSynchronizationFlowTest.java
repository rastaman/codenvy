...
public class LdapSynchronizationFlowTest {
  ...
  public void synchronization() throws Exception {
    ...
    assertEquals(
        userManager.getAll(3, 0).fill(new HashSet<>()), new HashSet<>(asList(user1, user2, user3)));
    ...
    assertEquals(syncResult.getFetched(), 3);
    assertEquals(
        userManager.getAll(3, 0).fill(new HashSet<>()), new HashSet<>(asList(user2, user3, user4)));
  ...
  public void conflictUpdateIsSolvedByTwoSynchronizations() throws Exception {
    ...
    assertEquals(syncResult.getFetched(), 2);
    assertEquals(
        userManager.getAll(2, 0).fill(new HashSet<>()), new HashSet<>(asList(user2, user3)));
  ...
  public static class Module extends AbstractModule {
    ...
    protected void configure() {
      ...
      bindConstant()
          .annotatedWith(Names.named("ldap.sync.user.filter"))
          .to("(objectClass=inetOrgPerson)");
      bind(String.class)
          .annotatedWith(Names.named("ldap.sync.group.additional_dn"))
          .toProvider(Providers.of(null));
      bind(String.class)
          .annotatedWith(Names.named("ldap.sync.group.filter"))
          .toProvider(Providers.of(null));
      bind(String.class)
          .annotatedWith(Names.named("ldap.sync.group.attr.members"))
          .toProvider(Providers.of(null));
      bind(String.class)
          .annotatedWith(Names.named("ldap.sync.user.additional_dn"))
          .toProvider(Providers.of(null));
      ...
      bind(new TypeLiteral<Pair<String, String>[]>() {})
          .annotatedWith(Names.named("ldap.sync.profile.attrs"))
