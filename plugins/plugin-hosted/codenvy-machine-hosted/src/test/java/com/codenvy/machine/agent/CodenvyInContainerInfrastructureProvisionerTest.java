...
public class CodenvyInContainerInfrastructureProvisionerTest {
  ...
  @Mock private AgentConfigApplier agentConfigApplier;
  ...
  public void
      shouldThrowEnvironmentExceptionIfAgentApplierThrowsAgentExceptionOnSingleMachineProvisioning()
          throws Exception {

    doThrow(new AgentException("test exception"))
        .when(agentConfigApplier)
        .apply(extendedMachine, cheService);
  ...
  public void shouldAddSyncKeyEnvVariableInDevMachineOnEnvProvision() throws Exception {
    ...
    assertEquals(
        internalEnv.getServices().get(DEV_MACHINE_NAME).getEnvironment().get("test"), "test1");
    assertEquals(
        internalEnv
            .getServices()
            .get(DEV_MACHINE_NAME)
            .getEnvironment()
            .get(CodenvyInContainerInfrastructureProvisioner.SYNC_KEY_ENV_VAR_NAME),
        SYNC_KEY);
  }
  ...
  public void shouldAddVolumeForProjectsFolderOnEnvironmentProvision() throws Exception {
    ...
    assertTrue(
        internalEnv.getServices().get(DEV_MACHINE_NAME).getVolumes().contains("/some/volume"));
    assertTrue(
        internalEnv
            .getServices()
            .get(DEV_MACHINE_NAME)
            .getVolumes()
            .contains("/some/bind:/mount/volume"));
  }

  @Test
  public void shouldAddVolumesForExcludedFromSnapshotFoldersOnEnvironmentProvision()
      throws Exception {
    provisioner.provision(environment, internalEnv);
    ...
    assertTrue(
        internalEnv
            ...
            .get(DEV_MACHINE_NAME)
            .getVolumes()
            .containsAll(SNAPSHOT_EXCLUDED_DIRECTORIES));
    assertTrue(
        internalEnv
            .getServices()
            .get(NON_DEV_MACHINE_NAME)
            .getVolumes()
            .containsAll(SNAPSHOT_EXCLUDED_DIRECTORIES));
  }
  ...
  public void
      shouldThrowEnvironmentExceptionIfAgentApplierThrowsAgentExceptionOnEnvironmentProvisioning()
          throws Exception {

    doThrow(new AgentException("test exception"))
        .when(agentConfigApplier)
        .apply(environment, internalEnv);
