...
/**
 * Auxiliary class for receive mail and search of the link for tenant creation
 * into email content. Class works only gmail boxes. IMAP must
...
public class MailReceiverUtils {
    ...
    /**
     * Creates session with email server with using IMAP - mail protocol.
     * Return array of the messages from the test mailbox
     * @param user login to test mail box that is used for mail connection
     * @param password password for test mailbox
     * @return array of messages from the mail box
     *  exceptions that may be produced during working with test mailbox:
    ...
    private Message[] receiveMailsFromInbox(String user, String password) throws MessagingException {
        ...
        mailStore.connect(host, user, password); //Connect to the current host using the specified username and password.
    ...
    /**
     * delete all mails in the test mailbox,
     * exceptions that may be produced during working with test mailbox:
     * @throws MessagingException
     */
     ...
     *
     * @param regexpPatterForLink predefined regexp for getting defined url in the body of mail.
     * It may be confirmation for workspace creation, reset password, add members into organizations and etc.
     * You can use patterns from TestConstants.RegexpPatternsForEmails class
     *
     * exceptions that may be produced during working with test mailbox:
     * @throws IOException
     * @throws MessagingException
     ...
     */
    public String getExpectedLinkFromEmail(String keyWordInMail, String regexpPatterForLink) throws IOException, MessagingException {
        ...
        for (int i = 0; i < messages.length; i++) {
            ...
            if (currentContent.contains(keyWordInMail) && currentMessage.getReceivedDate().before(new Date())) {
     ...
     * @param maxTimeAttempts The time during which will be performed attempts for getting expected mail
     ...
     */
    public String waitAndGetLink(int responseDelay, int maxTimeAttempts, String keyWordInMail, String regexpPatternForLink)
            throws Exception {
        ...
        if (confLink == null) throw new TimeoutException("There are no any messages from codenvy in mailbox");
        return confLink;
    }
