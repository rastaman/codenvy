...
public class ResourceAggregator {
    ...
    /**
     * Returns list which is result of deduction {@code resourceToDeduct} from {@code sourceResources}.
     *
     * @param sourceResources
     *         the source resources
     * @param resourcesToDeduct
     *         the resources which should be deducted from {@code sourceResources}
     * @throws NoEnoughResourcesException
     *         when {@code sourceResources} list doesn't contain enough resources
     * @throws IllegalArgumentException
     *         when {@code sourceResources} or {@code resourcesToDeduct} contain resource with not supported type
     */
    public List<? extends Resource> deduct(List<? extends Resource> sourceResources,
        ...
        final Map<String, Resource> result = sourceResources.stream()
    ...
    /**
     * Returns list which contains resources from specified {@code sourceResources}
     * which have excessive amount in compare to specified {@code resourcesToCompare}.
     *
     * <p>Example :
     * <pre>
     ...
     * @param sourceResources
     *         the source resources
     * @param resourcesToCompare
     *         the resources which should be compared to {@code sourceResources}
     * @throws IllegalArgumentException
     *         when {@code sourceResources} or {@code resourcesToCompare} contain resource with not supported type
     */
    public List<? extends Resource> excess(List<? extends Resource> sourceResources,
                                           List<? extends Resource> resourcesToCompare) {
        checkSupporting(sourceResources);
    ...
    /**
     * Returns resources list that contains resources with types that are contained by both input lists.
     *
     * @throws IllegalArgumentException
     *         when {@code resources} list contains resource with not supported type
     */
    public List<? extends Resource> intersection(List<? extends Resource> resourcesA, List<? extends Resource> resourcesB) {
        checkSupporting(resourcesA);
        checkSupporting(resourcesB);

        final Set<String> keysA = resourcesA.stream()
                                            .map(Resource::getType)
                                            .collect(toSet());
        final Set<String> keysB = resourcesB.stream()
                                            .map(Resource::getType)
                                            .collect(toSet());
    ...
    private void checkSupporting(Collection<? extends Resource> resources) {
        ...
        for (String resourcesType : resourcesTypes) {
            if (!this.resourcesTypes.containsKey(resourcesType)) {
                throw new IllegalArgumentException(String.format("'%s' resource type is not supported", resourcesType));
            }
        }
    }
    ...
    private ResourceType getResourceType(String typeId) {
        ...
        if (resourceType == null) {
            throw new IllegalArgumentException(String.format("'%s' resource type is not supported", typeId));
        }
        return resourceType;
    }
}
