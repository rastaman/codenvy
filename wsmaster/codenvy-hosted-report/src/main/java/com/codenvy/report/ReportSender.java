...
/**
 * Sends reports:
 * <li>to Codenvy with number of users.
 *
...
public class ReportSender {
  ...
  @Inject
  public ReportSender(
      @Named("report-sender.update_server_endpoint") String updateServerEndpoint,
  ...
  @ScheduleCron(
    cron = "0 0 1 ? * SUN *"
  ) // send each Sunday at 1:00 AM. Use "0 0/1 * 1/1 * ? *" to send every 1 minute.
  ...
  private void sendNumberOfUsers() throws IOException, JsonParseException, ApiException {
    ReportParameters parameters =
        obtainReportParameters(ReportType.CODENVY_ONPREM_USER_NUMBER_REPORT);
  ...
  private Ip obtainExternalIP()
    ...
    return httpJsonRequestFactory.fromUrl(requestUrl).request().asDto(Ip.class);
  }

  private ReportParameters obtainReportParameters(ReportType reportType)
      throws IOException, JsonParseException, ForbiddenException, BadRequestException,
          ConflictException, NotFoundException, ServerException, UnauthorizedException {
    String requestUrl =
        String.format(
            "%s/report/parameters/%s", updateServerEndpoint, reportType.name().toLowerCase());
