...
/**
 * Infrastructure provisioner that finds another infrastructure provisioner needed for current type
 * of installation and forward provisioning to it. Different strategies of provisioning are switched
 * by property {@value #INFRASTRUCTURE_TYPE_PROPERTY}.
 *
...
public class CodenvyInfrastructureProvisioner implements InfrastructureProvisioner {
  ...
  @Inject
  public CodenvyInfrastructureProvisioner(
      @Named(INFRASTRUCTURE_TYPE_PROPERTY) String codenvyInfrastructure,
    ...
    if (!infrastructureProvisioners.containsKey(codenvyInfrastructure)) {
      throw new RuntimeException(
          format(
              "Property '%s' has illegal value '%s'. Valid values: '%s'",
