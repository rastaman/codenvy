 ...
 *
 * <p>Root organizational account can use resources by itself or share
 * them for its suborganizations. So available resources equal to total
 * resources minus resources which are already used by organization or by
 * any of its suborganizations.
 *
 * <p>Suborganizational account can use all of parent resources or limited amount.
 * So available resource equal to minimum of parent available resources and parent
 * shared resources minus resources which are used by suborganization and its suborganizations.
 *
 * @author Sergii Leschenko
 */
@Singleton
public class OrganizationalAccountAvailableResourcesProvider implements AvailableResourcesProvider {
    private static final Logger LOG = LoggerFactory.getLogger(OrganizationalAccountAvailableResourcesProvider.class);
    ...
    @Inject
    public OrganizationalAccountAvailableResourcesProvider(Provider<ResourceUsageManager> resourceUsageManagerProvider,
    ...
    public List<? extends Resource> getAvailableResources(String accountId) throws NotFoundException, ServerException {
        ...
        if (organization.getParent() == null) {
            ...
            return resourceAggregator.min(resourceAggregator.intersection(getAvailableOrganizationResources(parentOrganization),
                                                                          getAvailableOrganizationResources(organization)));
        }
    }

    /**
     * Returns total resources minus resources which are already
     * used by organization or by any of its suborganizations.
     *
     * @param organization
     *         organization id to calculate its available resources
     * @return resources which are available for usage by specified organization
     * @throws NotFoundException
     *         when organization with specified id doesn't exist
     * @throws ServerException
     *         when any other exception occurs on calculation of available resources
     */
    @VisibleForTesting
    List<? extends Resource> getAvailableOrganizationResources(Organization organization) throws NotFoundException,
                                                                                                 ServerException {
        final ResourceUsageManager resourceUsageManager = resourceUsageManagerProvider.get();
        final List<? extends Resource> total = resourceUsageManager.getTotalResources(organization.getId());
        final List<Resource> unavailable = new ArrayList<>(resourceUsageManager.getUsedResources(organization.getId()));
        ...
        try {
            ...
            LOG.warn("Organization with id {} uses more resources {} than it has {}.", organization.getId(), format(unavailable),
                     format(total));
     ...
     *         parent qualified name, e.g. 'parentName/suborgName
     * @return resources which are used by suborganizations of specified organization.
     * @throws ServerException
     *         when any other exception occurs on calculation of used resources
     */
    @VisibleForTesting
    List<Resource> getUsedResourcesBySuborganizations(String parentQualifiedName) throws NotFoundException, ServerException {
        ...
        for (Organization suborganization : Pages.iterate((maxItems, skipCount)
                                                                  -> organizationManager.getSuborganizations(parentQualifiedName,
                                                                                                             maxItems,
                                                                                                             skipCount))) {
