...
public class ResourceAggregatorTest {
    ...
    @Mock
    private ResourceType aResourceType;
    @Mock
    private ResourceType bResourceType;
    @Mock
    private ResourceType cResourceType;
    ...
    public void shouldThrowConflictExceptionWhenTotalResourcesDoNotHaveEnoughAmountToDeduct() throws Exception {
        ...
        when(aResourceType.deduct(any(), any())).thenThrow(new NoEnoughResourcesException(singletonList(aResource),
                                                                                          singletonList(anotherAResource),
                                                                                          singletonList(new ResourceImpl(A_RESOURCE_TYPE,
                                                                                                                         222,
                                                                                                                         "unit"))));
    ...
    @Test(expectedExceptions = IllegalArgumentException.class)
    public void shouldThrowIllegalArgumentExceptionWhenTotalResourcesListContainsNotSupportedResourceOnResourcesDeduction()
            throws Exception {
        //given
        final ResourceImpl dResource = mock(ResourceImpl.class);
        when(dResource.getType()).thenReturn("resourceD");
    ...
    @Test(expectedExceptions = IllegalArgumentException.class)
    public void shouldThrowIllegalArgumentExceptionWhenResourcesToDeductListContainsNotSupportedResourceOnResourcesDeduction()
            throws Exception {
        //given
        final ResourceImpl dResource = mock(ResourceImpl.class);
        when(dResource.getType()).thenReturn("resourceD");
    ...
    public void shouldReturnIntersectionOfTwoResourcesLists() throws Exception {
        ...
        List<? extends Resource> intersection =
                resourceAggregator.intersection(asList(aResource, bResource), asList(anotherBResource, cResource));
