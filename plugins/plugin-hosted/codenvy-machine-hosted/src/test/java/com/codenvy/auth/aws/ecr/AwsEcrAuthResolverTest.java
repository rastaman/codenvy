...
public class AwsEcrAuthResolverTest {
  ...
  @Mock private AwsEcrInitialAuthConfig awsEcrInitialAuthConfig;
  ...
  private void before() {
    awsAccountCredentials =
        new AwsAccountCredentials(AWS_ID, AWS_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY);
  }
  ...
  private void beforeTest() {
    ...
    doReturn(AUTHORIZATION_TOKEN)
        .when(awsEcrAuthResolver)
        .getAwsAuthorizationToken(AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY);
  }

  @Test
  public void shouldBeAbleToGetDynamicXRegistryAuth() {
    AuthConfig authConfig = awsEcrAuthResolver.getXRegistryAuth(AWS_ECR);
  ...
  @Test
  public void
      shouldReturnNullWhenGetDynamicXRegistryAuthIfCredentialsForSpecifiedRegistryIsNotConfigured() {
  ...
  @Test
  public void
      shouldReturnNullWhenGetDynamicXRegistryAuthIfRetrievedAuthTokenCannotBeDecodedAsBase64() {
    doReturn("QVdTOmR5SomEvRoNgBaSe64VaLuE+=")
        .when(awsEcrAuthResolver)
        .getAwsAuthorizationToken(AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY);
  ...
  public void shouldGetDynamicXRegistryAuthOnlyForGivenRegistry() {
    Map<String, AwsAccountCredentials> configuredCredentials = new HashMap<>();
    configuredCredentials.put(
        "id1234.dkr.ecr.region-1.amazonaws.com",
        new AwsAccountCredentials("id1234", "region-1", "accessKey", "secretKey"));
    configuredCredentials.put(AWS_ECR, awsAccountCredentials);
    configuredCredentials.put(
        "102030.dkr.ecr.region-2.amazonaws.com",
        new AwsAccountCredentials("102030", "region-2", "accessKey2", "secretKey2"));
    when(awsEcrInitialAuthConfig.getAuthConfigs()).thenReturn(configuredCredentials);

    AuthConfig authConfig = awsEcrAuthResolver.getXRegistryAuth(AWS_ECR);

    verify(awsEcrAuthResolver, times(1))
        .getAwsAuthorizationToken(AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY);
  ...
  public void shouldBeAbleToGetDynamicXRegistryConfig() {
    Map<String, AuthConfig> authConfigsMap = awsEcrAuthResolver.getXRegistryConfig();
  ...
  public void shouldGetDynamicXRegistryConfigForAllConfiguredRegistries() {
    ...
    configuredCredentials.put(
        ...
        new AwsAccountCredentials("id1234", "region-1", "accessKey", "secretKey"));
    when(awsEcrInitialAuthConfig.getAuthConfigs()).thenReturn(configuredCredentials);
    doReturn("QVdTOmR5bmFtaWNQYXNzd29yZA==")
        .when(awsEcrAuthResolver)
        .getAwsAuthorizationToken("accessKey", "secretKey");
