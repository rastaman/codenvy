...
* `Homepage URL` : `http://<YOUR_IP_ADDRESS>:80`
* `Application description` : `Codenvy`
* `Authorization callback URL` : `http://<YOUR_IP_ADDRESS>:80/api/oauth/callback`

Substitute `CODENVY_GITHUB_CLIENT_ID` and `CODENVY_GITHUB_SECRET` properties in `codenvy.env` with `Client ID` and `Client Secret` taken from 
...
#### 4. Start Codenvy Onprem

Follow the guide: [https://github.com/codenvy/codenvy](https://github.com/codenvy/codenvy)
