...
/**
 * Machine security token service. Allows user to retrieve token to access to the particular
 * workspace, and, in the reverse case, allows get the user by his token.
 *
...
public class MachineTokenService {
   ...
   *
   * @param token token to find user by
   * @return user entity
   * @throws ApiException when token is not found, or there is problem retrieving user via api.
   * @throws IOException
   */
  @GET
  @Path("/user/{token}")
  ...
  public UserDto getUser(@PathParam("token") String token) throws ApiException, IOException {
    ...
    return requestFactory
        .fromUrl(apiEndpoint + "/user/" + userId)
        .useGetMethod()
        .request()
        .asDto(UserDto.class);
  }
